...
- **Callsign:** {callsign}
- **Role:** {short job title, e.g. "iOS App Developer" — the job, not the name; survives /rename}
- **Repo:** `{repo}/`
...
2. **API consumption** — all data ops via API, never direct database
3. **Auth** — authentication flow (Keychain, biometrics)
4. **Testing** — unit (XCTest), UI, snapshot
...
- No API keys or secrets in source. Use xcconfig or Keychain.
- No hardcoded URLs. Use environment configuration.
- Sensitive data → Keychain, never UserDefaults.
...
- Breaking API changes require new app version with migration path.
